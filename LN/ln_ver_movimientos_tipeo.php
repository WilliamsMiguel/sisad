<?php
// ln_ver_movimientos_tipeo.php
include '../AD/ad.php';

// Verificar si se ha enviado un filtro por nombre de receptor
$search = isset($_POST['search']) ? trim($_POST['search']) : '';

// Obtener los movimientos desde la base de datos con filtro por nombre de receptor
$movimientosElegida = obtener_movimientos_por_receptor($search);
...
foreach ($movimientosElegida as $movimiento) {
    ...
    if ($movimiento['estado_movimiento'] == 0) {
        $output .= '<td style="border: 1px solid #ffffff;">
                    <a href="../LN/ln_generar_documento.php?id_movimiento=' . $movimiento['id_movimiento'] . '" target="_blank">Generar documento para firmar</a>
                    <br><br>
        ...
        $output .= '<td style="border: 1px solid #ffffff;"><a href="../' . $movimiento['archivo_movimiento'] . '" target="_blank">Ver archivo</a></td>';
    }

    $output .= '<td style="border: 1px solid #ffffff;">
        <button class="btn btn-danger btn-sm editar" style="font-size: 10px;width: 80px">Editar</button>
        <button class="btn btn-primary btn-sm detalleVer" style="font-size: 10px;width: 80px">Ver Detalle</button>
    </td>';
    
    $output .= '</tr>';
    $output .= '<tr class="detalle-row" id="detalle-' . $movimiento['id_movimiento'] . '" style="border: #FFFFFF;">';
    $output .= '<td colspan="9">';
    $output .= '<div class="detalleContenido" style="vertical-align: top;"></div>';
    $output .= '</td>';
    $output .= '<td><span class="cerrarDetalle">&times;</span></td>';
    $output .= '</tr>';
}
